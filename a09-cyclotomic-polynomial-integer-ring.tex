...
\begin{itemize}
\item \textbf{The $\bm \mu$-th roots of unity} are the solutions for $X^\mu = 1$ over $X \in \mathbb{C}$ (complex numbers). The formula for the $\mu$-th root of unity is $X = e^{2 \pi i k / \mu}$ for all integers $k$ such that $0 \leq k \leq \mu - 1$. 
...
\textbf{Primitive}&&for any $1 \le d < \mu$ with $d \mid \mu$\\
\textbf{$\bm \mu$-th}&&\\
\textbf{Root of}&&\\
\textbf{Unity}&&\\\hline
...
\autoref{tab:cyclotomic-polynomial-comparison} compares the properties of the roots of unity and the $\mu$-th cyclotomic polynomial over $X \in \mathbb{C}$ (the complex numbers) and over $X \in \mathbb{Z}_p$ (the ring).
