...
$ \textcolor{white}{f(X) }= \sum\limits_{j=0}^{n} \ell_j(X) \cdot y_j$ \textcolor{red}{\# where $\ell_j(X) =  \sum\limits_{j=0}^{n} \left( \prod\limits_{\substack{0 \le k \le n\\ k \ne j}} \dfrac{X - x_k}{x_j - x_k}\right)$}
