...
$C = \textsf{GLWE}_{S, \sigma}(\Delta M) = ( A_0, A_1, \gap{$\cdots$}, A_{k-1}, B) \in \mathcal{R}_{\langle n,q \rangle}^{k + 1}$
...
$C + \Delta\Lambda = ( A_0, \text{ } A_1, \gap{$\cdots$}, A_{k-1}, \text{ } B + \Delta \Lambda)$
...
This means that $(A_0, A_1, ... \text{ } A_{k-1}, B + \Delta \Lambda)$ form the ciphertext $\textsf{GLWE}_{S, \sigma}(\Delta (\Lambda + M))$
\item Thus, \\
$\textsf{GLWE}_{S, \sigma}(\Delta M) + \Delta\Lambda$ \\
