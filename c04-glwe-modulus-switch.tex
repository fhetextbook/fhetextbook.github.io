...
\item \textbf{\underline{Encryption Input}:} $m \in \mathbb{Z}_t$, \text{ } $\vec{a} = (a_0, a_1, \ldots, a_{k-1}) \xleftarrow{\$} \mathbb{Z}_q^{k}$, \text{ } $e \xleftarrow{\chi_\sigma} \mathbb{Z}_q$

$ $

\item \textbf{\underline{Encryption}:} $\textsf{LWE}_{\vec{s},\sigma}(\Delta  m + e) = (\vec{a}, b) \in \mathbb{Z}_q^{k + 1}$ \text{ } (where $b = \vec{a} \cdot \vec{s} + \Delta  m + e \in \mathbb{Z}_q$)

$ $

\item \textbf{\underline{Decryption}:} $\textsf{LWE}^{-1}_{S,\sigma}(\textsf{ct}) = b - \vec{a}\cdot \vec{s} = \Delta  m + e \text{ } \text{ } \in \mathbb{Z}_q$
...
Given an LWE ciphertext $(\vec{a}, b)$ where $b = \vec{a}\cdot \vec{s} + \Delta m + e \bmod q$ and $m \in \mathbb{Z}_t$, modulus switch of the ciphertext from $q$ to $\hat q$ is equivalent to updating $(A, b)$ to $(\vec{\hat{a}}, \hat b)$ as follows:


$\vec{\hat{a}} = (\hat{a}_0, \hat{a}_1, \ldots, \hat{a}_{k-1})$, where each $\hat{a}_i = \Big\lceil a\dfrac{\hat{q}}{q} \Big\rfloor \in \mathbb{Z}_{\hat{q}}$ \textcolor{red}{ $\rhd$ $\lceil \rfloor$ means rounding to the nearest integer}
...
$\hat b - (\hat{a}_0s_0 + \hat{a}_1s_1 + \hat{a}_2s_2 + \hat{a}_3s_3) = 0(-12) + 1(6) + 1(-1) + 0(4) = 6 - 1 = 5 = \hat m + \hat e \in \mathbb{Z}_{32}$
...
Given a GLWE ciphertext $(\{A\}_{i=0}^{k-1}, B)$ where $B = \vec A\cdot \vec S + \Delta M + E \bmod q$ and $M \in \mathcal{R}_{\langle n, q \rangle}$, the modulus switch of the ciphertext from $q$ to $\hat q$ is equivalent to updating $(\{A\}_{i=0}^{k-1}, B)$ to $(\{\hat{A}\}_{i=0}^{k-1}, \hat B)$ as follows:
...
$\textsf{GLWE}_{{S},\sigma}(\hat{\Delta}  M + \hat{E} + E^{\langle \epsilon_{\textit{all}} \rangle}) = (\{\hat{A}\}_{i=0}^{k-1}, \hat{B}) \in \mathcal{R}_{\langle n, \hat{q} \rangle}^{k+1}$ 
