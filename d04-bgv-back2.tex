...
Remember that the requirement of modulus switch is that while we change the ciphertext modulus from $q$ to $\hat q$, it should decrypt to the same plaintext $M$. BGV's modulus switch is similar to that of RLWE modulus switch (\autoref{subsec:modulus-switch-rlwe}), but there are two additional requirements, because BGV applies the scaling factor $\Delta$ not to plaintext $M$, but to noise $E$. In the case of BFV or CKKS, their decryption process only needs to round off the noise in the LSB area. However, in the case of BGV, the plaintext is in the LSB area and its decryption process has to modulo off the noise in the higher-bit area by modulo reduction by $t$ (i.e., the plaintext modulus). More concretely, BGV's modulus switch from $(A, B \bmod q_{l}$ $\rightarrow$  $(\hat{A}, \hat{B} \bmod q_{l-1}$ should satisfy the decryption relation such that $(\hat{A} \cdot S + \hat{B} \bmod q_{l-1}) \bmod t = M$. 

To find the satisfactory $\hat{A}$ and $\hat{B}$, BGV's first step is to compute $\alpha$ and $\beta$ as follows:

$\alpha = t\cdot |(A \cdot t)^{-1}|_{\frac{q_l}{q_{l-1}}}$ \textcolor{red}{ \# $|(A\cdot t)^{-1}|_{\frac{q_l}{q_{l-1}}}$ is an inverse of $A\cdot t$ modulo $\frac{q_l}{q_{l-1}}$}

$\beta = t\cdot |(B \cdot t)^{-1}|_{\frac{q_l}{q_{l-1}}}$

$ $

Then, $\alpha$ satisfies the following relation:
...
, where $\alpha$ is used as some small value (between $-t\cdot\dfrac{q_l}{2q_{l-1}}$ and $t\cdot\dfrac{q_l}{2q_{l-1}}$) to make $A$ divisible by $\dfrac{q_l}{q_{l-1}}$. Similarly, we compute $\hat{B}$ as follows:


$\hat{B} = \dfrac{B - \beta}{\frac{q_l}{q_{l-1}}} \bmod q_{l-1}$

$ = \left|\left(\dfrac{q_{l}}{q_{l-1}}\right)^{-1}\right|_{q_{l-1}} \cdot (B - \beta) \bmod q_{l-1}$ 
\textcolor{red}{ \# since $\dfrac{q_l}{q_{l-1}}$ divides $B - \beta$, and $\dfrac{q_{l}}{q_{l-1}}$ and $q_{l-1}$ are co-prime}

$ $

$ $

Next, we derive the following relation;

$A \cdot S + B = M + \Delta E \pmod{q_l}$

$A \cdot S + B = M + \Delta E + Kq_l$

$(A - \alpha) \cdot S + (B - \beta) = M + \Delta E + Kq_l - \alpha S - \beta + Kq_l$

$w^{-1} \cdot (A - \alpha) \cdot S + w^{-1} \cdot (B - \beta) = w^{-1} \cdot M + w^{-1}\cdot \Delta E + w^{-1}\cdot Kq_l - w^{-1}\alpha S - w^{-1}\beta$ \textcolor{red}{ \# multiplying both sides by $w^{-1}$, where $w = \dfrac{q_l}{q_{l-1}}$}

$ $

$w^{-1} \cdot (A - \alpha) \cdot S + w^{-1} \cdot (B - \beta) = w^{-1} \cdot M + w^{-1}\cdot \Delta E + Kq_{l-1} - w^{-1}\alpha S - w^{-1}\beta$ \textcolor{red}{ \# substitute $w^{-1}\cdot Kq_{l} = \dfrac{q_{l-1}}{q_{l}}\cdot Kq_{l} = Kq_{l-1}$}

$ $

$w^{-1} \cdot (A - \alpha) \cdot S + w^{-1} \cdot (B - \beta) = w^{-1} \cdot M + w^{-1}\cdot \Delta E - w^{-1}\alpha S - w^{-1}\beta \pmod{q_{l-1}} $ 

\textcolor{red}{ \# we can do this since: (1) the left-hand side is guaranteed to be an integer, so the right-hand side must be an integer as well; (2) $w$ and $q_{l-1}$ are co-prime, so $w^{-1} \bmod q_{l-1}$ exists}

$ $

$\hat{A} \cdot S + \hat{B} = w^{-1} \cdot M + w^{-1}\cdot \Delta E \pmod{q_{l-1}} - w^{-1}\alpha S - w^{-1}\beta \pmod{q_{l-1}}$ \textcolor{red}{ \# by definition of $\hat{A}$ and $\hat{B}$}


$ $

Then, we get the following final decryption relation: 

$w^{-1} \cdot M + w^{-1}\cdot \Delta E - w^{-1}\alpha S - w^{-1}\beta \bmod p = M$ 

$ = M + \Delta E - \alpha - \beta \bmod p$ \textcolor{red}{ \# since $w^{-1} = \left(\dfrac{q_l}{q_{l-1}}\right)^{-1} \equiv 1 \bmod q$ (because $q_l \equiv q_{l-1} \equiv 1 \bmod p$)}

$ = M $ \textcolor{red}{ \# since $\alpha \equiv \beta \equiv 0 \bmod p$}

$ $

In order for the above final decryption step to hold, it must be the  case that $|w^{-1} \cdot M + w^{-1}\cdot \Delta E| < \dfrac{q_l}{2}$, otherwise they will overflow the $q_l$ modulus boundary and corrupt the plaintext message $M$ during modulo-$q_{l-1}$ reductions. As far as this noise threshold is satisfied, $(\hat{A}, \hat{B}) \bmod q_{l-1}$ is a valid ciphertext that satisfies the decryption relation:

$(\hat{A}\cdot S + \hat{B} \bmod q_{l-1}) \bmod p = M$

$ $

$\lfloor \frac{P}{Q} [x]_Q \rceil = \frac{P[x]_Q - t [P x t^{-1}]_Q}{Q} \equiv [- t [P x t^{-1}]_Q Q^{-1} ]_P$

We summarize BGV's modulus switch as follows:

I'd like to learn why $\textsf{ct}'(s) \bmod t = [m]_t$. Let's denote $\dfrac{q}{q'} = d$. Then, we can denote the prime number $\dfrac{q'}{q} \bmod q'$ as $d^{-1} \bmod q'$. 
In your slide, $\textsf{ct}'(s)$ has the term $\dfrac{q'}{q}\cdot [m]_t \bmod q'$, which is $d^{-1} \cdot [m]_t \bmod q'$.
I suppose that $( d^{-1} \cdot [m]_t \bmod q' ) \bmod t \neq [m]_t$, because $d^{-1} \bmod t \neq 1$.
Therefore, it's not intuitive for me why $\textsf{ct}'(s) \bmod t = [m]_t$.
...
Once the new ciphertext $(\hat A, \hat B) \bmod q_{l-1}$ is derived, then it decrypts to the same original plaintext polynomial $M$ as follows:
