...
\item \textbf{\underline{Slot Multiplication by \boldmath$|p^{-(\varepsilon-1)}|_{q_{l'}}$}:} To properly scale down the plaintext coefficients from $p^{\varepsilon-1}m_i \rightarrow m_i$ and the noise coefficients from $k''p^{\varepsilon} \rightarrow k''p$, we homomorphically multiply the slot values (resulted from the \textsf{SlotToCoeff} step) by $|p^{-(\varepsilon-1)}|_{q_{l'}}$. Here, we assume that $q_{l'}$ is the final ciphertext modulus after bootstrapping. Note that the inverse of $p^{(\varepsilon-1)} \bmod q_{l'}$ exists, because \textsf{gcd}$(p^{(\varepsilon-1)}, q_{l'}) = 1$. From the perspective of the ciphertext modulus $q_{l'}$, this modulo-$q_{l'}$ inverse multiplication results in $p^{-(\varepsilon-1)} \cdot (p^{\varepsilon-1}m_i + k''_ip^{\varepsilon}) = m_i + k''_{i}p \bmod q_{l'}$, successfully scaling down the coefficients of the plaintext and the noise polynomials. But for now, the output of this step is a ciphertext whose slots store $|p^{-(\varepsilon-1)}|_{q_{l'}} \cdot (p^{\varepsilon-1}m_i + k''_ip^{\varepsilon})$. 

$ $


\item \textbf{\textsf{\underline{SlotToCoeff}}:} This step works the same way as BFV's \textsf{SlotToCoeff} step: move $|p^{-(\varepsilon-1)}|_{q_{l'}} \cdot (p^{\varepsilon-1}m_i + k''_ip^{\varepsilon})$ stored in the input vector slots back to the polynomial coefficient positions by homomorphically multiplying with $\hathat W^*$. Upon completing this step, the ciphertext modulus becomes $q_{l'}$, smaller than $q_L$, because the homomorphic operation of step $2\sim7$ have consumed a few multiplicative levels. From the perspective of the ciphertext modulus $q_{l'}$, the result of this step is a ciphertext that encrypts the plaintext polynomial whose coefficients are $|p^{-(\varepsilon-1)}|_{q_{l'}} \cdot (p^{\varepsilon-1}m_i + k''_ip^{\varepsilon}) = m_i + k''_ip \bmod q_{l'}$, where both the message and the noise are properly scaled down. Importantly, $m_i + k''_ip \bmod q_{l'}$ is guaranteed to be within the modulo-$q_{l'}$ range. That is, $p^{\varepsilon-1}m_i + k''_ip^\varepsilon$ does not overflow or underflow $q_{l'}$, because $p^\varepsilon \ll q_{l'}$. The output of this step is $\textsf{RLWE}_{S, \sigma}(M + K''p) \bmod q_{l'}$, which also implicitly contains an additional noise term $E'p$ generated during the homomorphic operations between step $2 \sim 7$. 

$ $

\item \textbf{\textsf{\underline{Noise Term Re-interpretation}}:} We can view the $K''p$ and $E'p$ terms in $\textsf{RLWE}_{S, \sigma}(M + K''p) \bmod q_{l'}$ as the total noise of the ciphertext. In other words, we can view $\textsf{RLWE}_{S, \sigma}(M + K''p)$ with some noise term $E'p$ as a BGV ciphertext $\textsf{RLWE}_{S, \sigma}(M)$ with the noise term $E'p + K''p = (E' + K'')\cdot p$. This step does not require any additional computation. The size of the coefficients of $K''$ and $E'$ is upper-bounded because the operations of the \textsf{CoeffToSlot}, digit extraction, and \textsf{SlotToCoeff} steps are fixed. With a proper setup of the cryptographic parameters of BGV, we can guarantee that the noise-to-ciphertext modulus ratio always decreases after BGV's bootstrapping. In other words, $\dfrac{||E + K'||_{\infty}}{q_L} < \dfrac{||E' + K''||_{\infty}}{q_{l'}} < \dfrac{||E||_{\infty}}{\hat{q}}$, where $\hat{q} < q_l < q_{l'} < q_L$, and $||P||_\infty$ denotes the maximum absolute coefficient value of polynomial $P$.
...
The larger $\varepsilon$ is, the greater the (base-$p$) digit-wise gap between $p^{\varepsilon-1}M$ and $K'$ becomes; thus, the less likely it is that the decryption would fail (i.e., fail to zero out $K'$). However, a larger $\varepsilon$ means the digit extraction operation would be more expensive. 
