...
To eliminate $k'_i$ from the above (where $|k'_i| \leq n + 1$), we will use the same digit extraction polynomial $G_{\varepsilon, v}$ as in BFV (\autoref{subsubsec:bfv-bootstrapping-digit-extraction}) :

$z_i = d_0 + \left(\sum\limits_{j=\varepsilon'}^{\varepsilon-1} d_* p^j\right)$ \textcolor{red}{ $\rhd$ where $d_0 \in \mathbb{Z}_p$, \text{ } and $d_*$ can be any integer, \text{ } and $1 \leq \varepsilon' \leq w$}
...
, where $k_i''p^\varepsilon$ is some multiple of $p^\varepsilon$ to account for the original $p^\varepsilon$-overflow term plus an additional $p^\varepsilon$-overflows generated during the digit extraction. Note that the digit extraction step reduces the ciphertext modulus from $q_L \rightarrow q_{l'}$ (where $l'$ is an integer smaller than $L$), because the homomorphic evaluation of the polynomial $G_{\varepsilon, v}$ requires some ciphertext-to-ciphertext multiplications, which consume some multiplicative levels. The output of the digit extraction step is $p^{\varepsilon-1}m_i + k''_ip^\varepsilon$ stored in each input vector slot. 

$ $



\item \textbf{\textsf{\underline{SlotToCoeff}}:} This step works the same way as BFV's \textsf{SlotToCoeff} step: move $p^{\varepsilon-1}m_i + k''_ip^{\varepsilon}$ stored in the input vector slots back to the polynomial coefficient positions by homomorphically multiplying with $\hathat W^*$. Upon completing this step, the ciphertext modulus is some value $q_{l'}$ smaller than $q_L$, because the homomorphic operation of step $2\sim7$ has consumed a few multiplicative levels. The resulting plaintext coefficients are $p^{\varepsilon-1}m_i + k'''_ip^{\varepsilon}$, where $k''' > k''$ due to the additional noise generated by homomorphically running the \textsf{SlotToCoeff} step. The plaintext modulus (i.e., the noise scaling factor) is still $p^{\varepsilon}$ at this point. We let these polynomials $M$ and $K'''$ each, and the output ciphertext is $\textsf{RLWE}_{S, \sigma}(Mp^{\varepsilon - 1} + K'''p^{\varepsilon}) \bmod q_{l'}$.

$ $

\item \textbf{\underline{Ciphertext Coefficient Multiplication by \boldmath$|p^{-(\varepsilon-1)}|_{q_{l'}}$}:} To properly scale down the plaintext coefficients from $p^{\varepsilon-1}m_i \rightarrow m_i$ and the noise coefficients from $k'''p^{\varepsilon} \rightarrow k'''p$, we multiply the ciphertext's polynomial coefficients by $|p^{-(\varepsilon-1)}|_{q_{l'}}$. Note that the inverse of $p^{(\varepsilon-1)} \bmod q_{l'}$ exists because \textsf{gcd}$(p^{(\varepsilon-1)}, q_{l'}) = 1$. This operation results in $p^{-(\varepsilon-1)} \cdot (p^{\varepsilon-1}m_i + k'''_ip^{\varepsilon}) = m_i + k'''_{i}p \bmod q_{l'}$, successfully scaling down the coefficients of the plaintext and the noise polynomials. The output ciphertext is $\textsf{RLWE}_{S, \sigma}(M + K'''p) \bmod q_{l'}$.
...
\subsubsection{Noise Growth Reduction by Early Inverse Multiplication} 

It is possible to perform the inverse multiplication step before the \textsf{SlotToCoeff} step to mitigate noise growth. In the standard sequence, the inputs to \textsf{SlotToCoeff} are scaled by a large factor $p^{\varepsilon-1}$, meaning that the noise accumulated during the matrix-vector multiplication of \textsf{SlotToCoeff} is proportional to this large scaling factor. By moving the inverse multiplication step earlier (specifically, executing it on the packed slot values immediately after digit extraction), we scale down the message and noise coefficients to their minimal size (proportional to 1 rather than $p^{\varepsilon-1}$) before they enter the expensive \textsf{SlotToCoeff} transformation. This ensures that the homomorphic linear transformation operates on smaller values, significantly reducing the additional noise generated during the change of basis and preserving a larger noise budget for subsequent computations. 

Note that if we perform the inverse multiplication at this stage, the scaling factor must be computed as the inverse modulo $q_{\hat{l}}$, where $q_{\hat{l}}$ denotes the specific ciphertext modulus at the end of the digit extraction step (rather than the final modulus $q_{l'}$). Furthermore, although we are conceptually scaling the values stored in the slots, we do not need to encode the inverse into a plaintext vector. Due to the linearity of the scheme, simply multiplying the ciphertext's polynomial coefficients by the scalar integer $|p^{-(\varepsilon-1)}|_{q_{\hat{l}}}$ effectively scales the underlying slot values efficiently.
...
Both BFV and BGV's bootstrapping uses digit extraction, but for different purposes. In BFV, digit extraction is used to eliminate the noise in the lower-bit area. In BGV, digit extraction is used to eliminate the modulus garbage values in the lower-bit area generated by \textsf{ModRaise} from $\hat{q}\rightarrow q_L$. In addition, at the end of BGV's digit extraction, we homomorphically multiply the slot value by $|p^{-(\varepsilon-1)}|_{q_{l'}}$ to scale down both the message and the noise to the proper value. In BFV, this additional operation step is unnecessary because we only need to re-interpret the scaling factor. 
