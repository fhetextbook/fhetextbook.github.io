...
%\textcolor{red}{ $\rhd$ where $\hat{y}_i = \dfrac{qb}{q_i}$, $\hat{y}'_i = \dfrac{qb}{b_i}$, $\hat{z}_i = \hat{y}^{-1}_i$, and $\hat{z}'_i = \hat{y}'^{-1}_i$}
...
%$= \left|\dfrac{t\cdot |\textsf{ct}(s)|_q - |t\cdot \textsf{ct}(s)|_q}{q}\right|_t + e_r + e_d$ \textcolor{red}{ $\rhd$ assuming $e_r$ and $e_d$ are small enough not to cause any $t$-overflow}

%$= \left|(t\cdot |\textsf{ct}(s)|_q - |t\cdot \textsf{ct}(s)|_q)\cdot q^{-1}\right|_t + e_r + e_d$ \textcolor{red}{ $\rhd$ since $q$ divides $t\cdot |\textsf{ct}(s)|_q - |t\cdot \textsf{ct}(s)|_q$}

%$= \left|- |t\cdot \textsf{ct}(s)|_q\cdot q^{-1}\right|_t + e_r + e_d$ \textcolor{red}{ $\rhd$ since $t\cdot |\textsf{ct}(s)|_q$ is a multiple of $t$}

%$= ||t\cdot \textsf{ct}(s)|_q|_t \cdot |-q^{-1}|_t + e_r + e_d$

%$= \bm(\textsf{FastBConv}(t\cdot \textsf{ct}(s), q, t) - uq\bm) \cdot |-q^{-1}|_t + e_r + e_d$ \textcolor{red}{ $\rhd$ note that the fast base conversion of $t\cdot \textsf{ct}(s)$ from $q\rightarrow t$ is $\textsf{FastBConv}(t\cdot \textsf{ct}(s), q, t) = t\cdot \textsf{ct}(s) + uq \bmod t$, where integer $|u| \leq \dfrac{l}{2}+1$ and the original base moduli are ${q_1, q_2, \cdots, q_l}$.}
...
%$= \bm(\textsf{FastBConv}(\gamma\cdot t\cdot \textsf{ct}(s), q, \{t,\gamma \}) - u'q\bm) \cdot |-q^{-1}|_{\gamma t} + e'_r + e'_d$ \textcolor{red}{ $\rhd$ note that the fast base conversion of $\gamma\cdot t\cdot \textsf{ct}(s)$ from $q\rightarrow t\cdot \gamma$ is $\textsf{FastBConv}(\gamma\cdot t\cdot \textsf{ct}(s), q, \{t, \gamma\}) = \gamma\cdot t\cdot \textsf{ct}(s) + u'q \bmod \gamma t$, where $|u'| \leq \dfrac{l}{2}+1$ and the original base moduli are ${q_1, q_2, \cdots, q_l}$.}
...
%= |(\textsf{FastBConv}(\hat{x}, b, b_\alpha) - x_\alpha)\cdot b^{-1}|_{b_\alpha}$ \textcolor{red}{ $\rhd$ $ = |(\textsf{FastBConv}(x, b, b_\alpha) - x_\alpha)\cdot b^{-1}|_{b_\alpha} = |(\textsf{FastBConv}(\hat{x}, b, b_\alpha) - x_\alpha)\cdot b^{-1}|_{b_\alpha}$}
...
%$\textsf{ModRaise\textsubscript{RNS}}(x, q, qbb_\alpha)$ \textcolor{red}{ $\rhd$ where $b > q$ and $b \approx q$, and $x$ one coefficient of a ciphertext polynomial} 

%$= (x_1, x_2, \cdots, x_{q_k}, \textsf{FastBConv}(x, q, b))$

%$= (x_1, x_2, \cdots, x_{q_k}, c_1, c_2, \cdots, c_l)$ \textcolor{red}{ $\rhd$ where $c = x + uq \bmod b$ (where integer $|u| \leq \dfrac{k}{2}+1$)}
...
%, where $\hat\nu = \Big||tx + tuq|_{q} + u'q\Big|_b = \Big||tx|_{q} + u'q\Big|_b$ \textcolor{red}{ $\rhd$ $|u'| \leq \dfrac{l}{2}+1$}
...
%$ = \left|\dfrac{tx - |tx|_{q}}{q} + tu - u')\right|_b$ \textcolor{red}{ $\rhd$ where $\dfrac{tx - |tx|_{q}}{q}$ is an integer}
...
%$\beta_{\hat{\tau}} = (\textsf{FastBConv}(\hat{\tau}, \hat{b}, b_\alpha) - \hat{\tau})\cdot \hat{b}^{-1}$ \textcolor{red}{ $\rhd$ where $b_\alpha = \dfrac{b}{\hat b}$ is one prime factor of the base $b$}
...
%$ \approx \dfrac{x}{\Delta} + tu + u'$ \textcolor{red}{ $\rhd$ where $u$ and $u'$ can be reduced down to $\{-1, 0, 1\}$ by using \textsf{SmallMont} (\autoref{subsec:rns-smallmont})}
...
$A''\cdot S + B'' = M + \hat{q}pE'' + K\hat{q}$ \textcolor{red}{ $\rhd$ the noise factor increased from $E \rightarrow E''$, and the message correction factor $\kappa = \hat{q}$}
...
$A'''\cdot S + B''' = p^{\varepsilon-1}M + p^\varepsilon \hat{q}pE'' + Kq_l\hat{q}$ \textcolor{red}{ $\rhd$ the noise factor increased from $E \rightarrow E''$, and the message correction factor is still $\kappa = \hat{q}$}

$ $

$ = p^{\varepsilon-1}M + p^\varepsilon E''' + K'''\hat{q} \pmod{\hat{q}}$ \textcolor{red}{ $\rhd$ where we denote $E''' = \hat{q}pE''$, and reduce $Kq_l \equiv K''' \bmod \hat{q}$, where $K''' \leq n$}
...
$A' \cdot S + B' = p^{\varepsilon-1}M + p^\varepsilon E + Kq_l$ \textcolor{red}{ $\rhd$ $||K||_\infty \leq n$ ($||K||_\infty$ denotes the maximum coefficient of $K$)}
...
$A'' \cdot S + B'' = p^{\varepsilon-1}M + p^\varepsilon E + Uq_l \pmod{q_l\hat{q}}$ \textcolor{red}{ $\rhd$ where $Uq_l$ includes $Kq_l$ plus the noise generated by \textsf{ModRaise\textsubscript{RNS}}'s \textsf{FastBConv} with \textsf{SmallMont}}
...
$A'''S + B''' = p^{\epsilon-1}M + p^\varepsilon E' + U \pmod{\hat{q}}$ \textcolor{red}{ $\rhd$ $E\rightarrow E'$ is an increase due to BGV's \textsf{ModSwitch} noise}
...
$A''''\cdot S + B'''' = p^{\varepsilon-1} M + p^{\varepsilon}E + U + U'\hat{q} \pmod{q_L}$ \textcolor{red}{ $\rhd$ where $U'\hat{q}$ is a noise generated by this step}
