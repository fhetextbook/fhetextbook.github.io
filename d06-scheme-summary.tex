...
&4. \underline{Digit Extraction}: Given the digit extraction polynomial $G_{\varepsilon, v}(x)$,\\
...
&5. \underline{Digit Extraction}: Given the digit extraction polynomial $G_{w, v}(x)$, \\
&\text{ } \text{ }  homomorphically compute:\\
...
&\text{ } \text{ } , and then the output $Mp^{\varepsilon - 1} + K''p^\varepsilon$ is stored in the plaintext slots.\\
&6. \underline{Ciphertext Coefficient Multiplication by $|p^{-(\varepsilon-1)}|_{q_{\hat{l}}}$}: Multiply $|p^{-(\varepsilon-1)}|_{q_{\hat{l}}}$ to all \\
&\text{ } \text{ }  ciphertext polynomial coefficients to update their plaintext portion from\\
&\text{ } \text{ }  $Mp^{\varepsilon - 1} + K'''p^\varepsilon \pmod{q_{\hat{l}}}$ to $M + K'''p \pmod{q_{\hat{l}}}$\\
&\text{ } \text{ }   \textcolor{red}{ $\rhd$ where $q_{\hat{l}}$ is the ciphertext modulus at the end of step 5}\\
&7. \underline{\textsf{SlotToCoeff}}: Multiply to the ciphertext by $\hathat W^*$ to move $M + K''p$ to the \\
&\text{ } \text{ } polynomial coefficient positions to get $\textsf{RLWE}_{S, \sigma}(Mp^{\varepsilon - 1} + K'''p^{\varepsilon}) \bmod q_{l'}$.\\
&8. \underline{Noise Term Re-interpretation}: View $\textsf{RLWE}_{S, \sigma}(M + K'''p) \bmod q_{l'}$ as\\
&\text{ } \text{ }  $\textsf{RLWE}_{S, \sigma}(M) \bmod q_L$, an encryption of $M$ with the new noise $E = K'''$\\
& \text{ } \text{ }  having the noise scaling factor (i.e., plaintext modulus) $\Delta = p$.\\\hline
\end{tabular}
