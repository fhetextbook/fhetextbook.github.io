...
N = 8
P = 17
bfv_vector = [1, 2, 3, 4, 5, 6, 7, 8]
bfv_vector2 = [2, 4, 3, 6, 0, 0, 0, 0]
rotation_offset = 3
...
def decode(self, p: Polynomial) -> np.array:
	...
	print('descaled rotation-applied polynomial: ' + str(p))
	print()
	#print(self.sigma_R_basis)
...
def main():
	...
	p = encoder.encode(z)
	p_rotated = rotate_homomorphic(p, rotation_offset, N) # divide by a redundant scale factor
	...
	print('after polynomial rotation by ' + str(rotation_offset) + ' positions and q-reduction of polynomial 1: ' + str(p_rotated))

	decoded_z_rotated = encoder.decode(p_rotated)
	print("<The decoded BFV plaintext vector>")
	print('rotated vector 1 by ' + str(rotation_offset) + ' positions: ' + str(decoded_z_rotated))
	print()

	for i in range(len(z)//2):
		original_index = i
		rotated_index = (i - rotation_offset) % (len(z)//2)
		if z[original_index] != decoded_z_rotated[rotated_index] or z[original_index + len(z)//2] != decoded_z_rotated[rotated_index + len(z)//2]:
